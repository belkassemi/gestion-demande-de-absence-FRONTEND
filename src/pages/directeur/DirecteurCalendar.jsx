...
import { useGetDirecteurCalendarQuery } from '../../features/api/absenceApi';
import { Clock, CheckCircle, X, Building2 } from 'lucide-react';
import { formatDate } from '../../lib/utils';

// A curated palette to assign one color per department
const DEPT_PALETTE = [
...
export default function DirecteurCalendar() {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h1 style={{ fontSize: '1.375rem', fontWeight: 700, color: '#0f172a', marginBottom: '2px' }}>
            Calendrier des absences
          </h1>
          <p style={{ fontSize: '13px', color: '#64748b' }}>Vue globale de toutes les absences par département</p>
        </div>
      ...
      {departments.length > 0 && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', marginBottom: '1.25rem' }}>
          {departments.map((d) => (
            <button
              ...
              style={{
                ...
                padding: '5px 12px', borderRadius: '20px', border: `2px solid ${filterDept === String(d.id) ? d.color : 'transparent'}`,
                background: `${d.color}18`, fontSize: '12px', fontWeight: 600, color: d.color, cursor: 'pointer',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px', color: '#94a3b8', padding: '5px 0', borderLeft: '1px solid #e2e8f0', paddingLeft: '12px', marginLeft: '2px' }}>
            <span style={{ width: 8, height: 8, borderRadius: '2px', background: 'currentColor', display: 'inline-block', opacity: 0.5 }} />
            Bordure = En attente
          </div>
        </div>
      )}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', background: '#f8fafc', borderRadius: '12px', padding: '1rem' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px' }}>
                ...
                <span style={{ fontWeight: 700, color: '#1e293b' }}>
                  {formatDate(selectedEvent.extendedProps.start)} → {formatDate(selectedEvent.extendedProps.end)}
                </span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px' }}>
